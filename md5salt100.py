...
def rainbow(): #make a rainbow table
    ...
    if os.path.exists(dictionary):
        ...
        with open(dictionary, "r") as wordlist:
            row_count = sum(1 for row in wordlist)
            wordlist.seek(0)
            end = time.time()
            print("finding rows took", end - start, "seconds")
            with open("rainbow.txt", "w") as rainbow: #output file
                ...
                for row in wordlist:
                    ...
                    for i in range(sfloor, sceiling):
                        ...
                        rainbow.write("%s$%s\n" % (password, md5))
                    if int((counter / row_count) * 100) != percentage:
                        percentage = int((counter / row_count) * 100)
                        clear()
